...
class parameter_node : public ast_node {
  ...
  public:
    parameter_node(pattern_node *pattern, type_node *type) noexcept
...
class fn_node : public ast_node {
  ...
  public:
    fn_node(identifier_node *name, std::vector<parameter_node *> parameters,
            block_expression_node *code) noexcept
...
class statement_node : public ast_node {
  private:
    ...
    std::optional<expression_node *>
        expression; // Includes expression_without_block
  public:
    statement_node(std::optional<item_node *> item,
                   std::optional<let_statement_node *> let_statement,
                   std::optional<expression_node *> expression) noexcept
        : item(item), let_statement(let_statement), expression(expression) {}
    [[nodiscard]] std::optional<item_node *> get_item() const noexcept;
    [[nodiscard]] std::optional<let_statement_node *>
    get_let_statement() const noexcept;
    [[nodiscard]] std::optional<expression_node *>
...
class block_expression_node : public ast_node {
  ...
  public:
    block_expression_node(
        std::vector<statement_node *> statements,
        std::optional<expression_without_block_node *> ret_val) noexcept
        : statements(statements), ret_val(ret_val) {}
    [[nodiscard]] std::vector<statement_node *> get_statements() const noexcept;
    [[nodiscard]] std::optional<expression_without_block_node *>
    get_ret_val() const noexcept;
};
